...
import static org.openhab.binding.skyq.internal.SkyQBindingConstants.THING_TYPE_SKYQ;
import static org.openhab.binding.skyq.internal.SkyQConfiguration.HOSTNAME;
...
/**
 * The {@link SkyQDiscoveryParticipant} is responsible for processing the
...
@NonNullByDefault
@Component
public class SkyQDiscoveryParticipant implements UpnpDiscoveryParticipant {
    private final Logger logger = LoggerFactory.getLogger(SkyQDiscoveryParticipant.class);

    @Override
    public Set<ThingTypeUID> getSupportedThingTypeUIDs() {
        return Collections.singleton(THING_TYPE_SKYQ);
    }
    ...
    public @Nullable DiscoveryResult createResult(RemoteDevice device) {
        ...
        if (uid != null) {
            ...
            properties.put(HOSTNAME, device.getIdentity().getDescriptorURL().getHost());

            DiscoveryResult result = DiscoveryResultBuilder.create(uid).withProperties(properties)
                    .withRepresentationProperty(HOSTNAME).withLabel(getLabel(device)).build();
        ...
        if (device.getDetails() != null && device.getDetails().getManufacturerDetails() != null) {
            ...
            if (manufacturer != null && manufacturer.toUpperCase().contains("SAMSUNG ELECTRONICS")) {
                ...
                if (device.getType() != null && "MediaRenderer".equals(device.getType().getType())) {
                    ...
                    return new ThingUID(THING_TYPE_SKYQ, udn);
                }
            }
        }
        return null;
